...
# N_CLUSTERS = 4 (Para tentar alinhar com os 4 canais)
km = KModes(n_clusters=2, init="Huang", n_init=3, verbose=0)
clusters = km.fit_predict(df_work[cols_to_analyze])
df_work["Cluster_Label"] = clusters.astype(
    str
)  # String para o Plotly entender como categórico
...
# Aqui a mágica acontece: Pontos da mesma COR são do mesmo grupo estatístico.
# Pontos da mesma FORMA (símbolo) têm o mesmo risco.
# Se um Cluster (Cor) tiver misturas de Formas, ele não é puro em relação ao risco.
fig3 = px.scatter(
    df_work,
    x="MCA_X",
    y="MCA_Y",
    color="Cluster_Label",  # Cor define o Cluster
    symbol="Target_Channel",  # Formato define o Risco (Verde/Vermelho)
    title="Visão 3: Sobreposição (Cor=Cluster, Forma=Risco)",
    hover_data=common_hover,
    opacity=0.8,
    height=800,  # Maior para ver melhor
)
...
print("\nConcluído! Abra a pasta 'eda_html_reports' e compare os 3 arquivos.")
